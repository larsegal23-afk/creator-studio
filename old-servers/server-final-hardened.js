...
app.post("/api/generate-logo", requireAuth, async (req, res) => {
  try {
    ...
    // DEDUCT COINS SERVER SIDE - TRANSACTION
...
// Stripe webhook - 100% HARDENED - NO FALLBACKS
app.post("/api/stripe/webhook", express.raw({ type: "application/json" }), async (req, res) => {
  try {
    const sig = req.headers["stripe-signature"]
    
    // MUST have webhook secret - NO FALLBACKS
    ...
    if (process.env.STRIPE_WEBHOOK_SECRET.includes("placeholder")) {
      console.error("❌ Webhook secret is placeholder")
      return res.status(500).json({ error: "Webhook not configured" })
    }
...
app.listen(process.env.PORT || 3000, () => {
  console.log("🚀 === FINAL HARDENED PRODUCTION SERVER ===")
