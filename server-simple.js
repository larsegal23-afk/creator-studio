...
// Helper function to check and deduct coins
const checkAndDeductCoins = async (userId, amount, description) => {
  ...
  return await db.runTransaction(async (transaction) => {
    ...
    if (!userDoc.exists) {
      ...
      throw new Error("User created with 0 coins - must purchase")
    }
...
app.post("/api/use-coins", requireAuth, async (req, res) => {
  try {
    ...
    const result = await checkAndDeductCoins(userId, amount, "Manual coin usage")
    
    console.log(`User ${userId} used ${amount} coins`)
    res.json(result)
...
app.post("/api/generate-logo", requireAuth, async (req, res) => {
  try {
    ...
    // MANDATORY: Check and deduct 5 coins
...
app.post("/api/create-stream-pack", requireAuth, async (req, res) => {
  try {
    ...
    // MANDATORY: Check and deduct 15 coins
...
app.post("/api/process-video", requireAuth, async (req, res) => {
  try {
    ...
    // MANDATORY: Check and deduct coins
...
app.get("/api/health", (req, res) => {
  res.json({ 
    ...
    message: "Mandatory coins system active - NO FALLBACKS"
  })
})
